...
- **Status:** in progress — the default algorithm is now complete. `@text-rendering-toolkit/linebreak` implements UAX #14 revision 55 for Unicode 17.0.0 as ordered rules over explicit carried state, passes all 19,338 official conformance cases, and is swapped in behind the layout adapter with no recorded fixture change. Remaining browser-grade work is tailoring rather than algorithm: CSS/locale `line-break`/`word-break`, dictionary segmentation for complex-context scripts, hyphenation, and browser parity evidence.
- **Confidence:** high
...
- **Open questions:** Which browser fixtures define parity? Which scripts require reshaping around an accepted break? Does `bidi-js` at Unicode 13.0.0 now become the limiting inconsistency?
...
- 2026-07-28: Replaced pair-table line breaking with a project-owned rule-based implementation. `@text-rendering-toolkit/linebreak` implements UAX #14 revision 55 for Unicode 17.0.0 — all 44 rules evaluated in specification order against explicit carried state, with property tables generated from vendored UCD data and committed. It passes 19,338/19,338 official conformance cases with none excluded. LB25 is decided from forward-carried state rather than a backward scan, which also makes a streaming driver viable: it matches batch output across all 83,716 two-way corpus splits, withholds decisions needing unarrived input, and retained 5 UTF-16 units peak over a 108,000-unit stream while running at 0.79-1.05x batch cost. The layout adapter swap changed no recorded fixture and left `PreparedText` schema version 2 intact; `linebreak@1.1.0` and its Unicode 13 data are removed.
