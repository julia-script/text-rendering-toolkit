...
const trackedCleanups = new WeakMap();
function track(obj, destroy) {
	const ptr = obj.ptr;
	const cleanup = () => destroy(ptr);
	trackedCleanups.set(obj, cleanup);
	registry.register(obj, cleanup, obj);
}
function destroyTracked(obj) {
	const cleanup = trackedCleanups.get(obj);
	if (!cleanup) return;
	registry.unregister(obj);
	trackedCleanups.delete(obj);
	cleanup();
}
/**
...
function typed_array_from_set(setPtr) {
	...
	const result = new Uint32Array(Module.HEAPU32.subarray(arrayOffset, arrayOffset + setCount));
	exports.free(arrayPtr);
	return result;
}
//#endregion
//#region src/types.ts
...
var Font = class Font {
	constructor(arg) {
		...
		trackedCleanups.set(this, cleanup);
		registry.register(this, cleanup, this);
	}
...
//#endregion
export { Blob, Buffer, BufferContentType, BufferFlag, BufferSerializeFlag, BufferSerializeFormat, ClusterLevel, Direction, Face, Feature, Font, FontFuncs, GlyphClass, GlyphFlag, MetricsTag, TracePhase, Variation, destroyTracked as __destroyTracked, getRuntime as __getRuntime, otTagToLanguage, otTagToScript, shape, shapeWithTrace, version, versionString };
